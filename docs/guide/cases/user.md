...
@cmd.handle()
async def _():
    await db.cursor.execute('SELECT username, email FROM User WHERE age > 30')
    users = await db.cursor.fetchall()
    for user in users:
        await cmd.send(f'{user[0]}, {user[1]}')
    await cmd.finish()
```
...
不过我们还没有正式的开始构建我们的用户系统。在下一节中，我们来学习定义一个数据库模型 `Model`。
...
`created_at` 字段被声明为 `datetime` 类型，并且将数据的创建时间设置为默认值。
...
```sql
CREATE TABLE nonebot_plugin_test_user (
        id INTEGER NOT NULL,
        user_id VARCHAR NOT NULL,
        last_use DATETIME NOT NULL,
        created_at DATETIME NOT NULL,
        CONSTRAINT pk_nonebot_plugin_test_user PRIMARY KEY (id)
)
```
...
[('add_table',
  Table('nonebot_plugin_test_user', MetaData(), Column('id', Integer(), table=<nonebot_plugin_test_user>, primary_key=True, nullable=False), Column('user_id', String(), table=<nonebot_plugin_test_user>, nullable=False), Column('last_use', DateTime(), table=<nonebot_plugin_test_user>, nullable=False), Column('created_at', DateTime(), table=<nonebot_plugin_test_user>, nullable=False, default=CallableColumnDefault(<function datetime.now at 0x7fd632a04940>)), schema=None))]
```
